...
export default function App() {
  ...
  useEffect(() => {
    const obtenerBarberias = async () => {
      setCargando(true);
      try {
        const respuesta = await fetch("http://127.0.0.1:8000/api/barberias");
        if (respuesta.ok) setBarberias(await respuesta.json());
      } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-[#060b14] dark:text-slate-200 font-sans transition-colors duration-300">
      ...
      {vistaActual === "dashboard" && (
        <AdminDashboard usuario={usuario} onVolver={() => setVistaActual("tienda")} />
      )}
      ...
      {vistaActual === "tienda" && (
        <div className="animate-fade-in flex flex-col min-h-screen">
          ...
          <nav className="fixed top-0 w-full z-40 bg-white/80 border-b border-slate-200 dark:bg-[#03070e]/80 dark:border-slate-800/50 backdrop-blur-md transition-colors duration-300">
            <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                <button onClick={() => setTemaOscuro(!temaOscuro)} className="p-2 rounded-full text-slate-500 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-700 transition-colors" title="Cambiar Tema">
          ...
          {!barberiaSeleccionada ? (
            ...
            /* --- VISTA 2: CATÁLOGO DE UNA BARBERÍA ESPECÍFICA --- */
            <>
      ...
      {servicioSeleccionado && barberiaSeleccionada && (
        <BookingModal
          ...
          barberiaSlug={barberiaSeleccionada.slug} // Pasamos el slug de la barbería seleccionada
