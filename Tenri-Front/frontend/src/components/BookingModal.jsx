    ...
    const obtenerHorasOcupadas = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // Generamos los bloques de 1 hora según el turno del barbero
          ...
          for (let i = inicioNum; i <= finNum; i++) {
            if (i === 14) continue; // Bloqueo de colación
      ...
      <div className="bg-white dark:bg-[#0B1221] border border-slate-200 dark:border-slate-800/60 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh] transition-colors">
        ...
        <div className="px-8 py-6 border-b border-slate-200 dark:border-slate-800/60 flex justify-between items-start bg-slate-50 dark:bg-[#080d18]">
          <div className="flex gap-5 items-center">
            {servicio.imagen_url ? (
              <img src={servicio.imagen_url} alt={servicio.nombre} className="w-20 h-20 rounded-xl object-cover border border-slate-200 dark:border-slate-700/50 shadow-sm" />
            ) : (
          ...
          <button onClick={onClose} className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-200 dark:text-slate-500 dark:hover:text-rose-400 dark:hover:bg-slate-800/50 rounded-lg transition-colors shrink-0 mt-1"><XIcon /></button>
        </div>
        ...
        <div className="p-8 overflow-y-auto custom-scrollbar flex-1">
          <form id="form-reserva" onSubmit={handleReservar} className="space-y-8">
              ...
              <div className="grid grid-cols-2 gap-3">
                {barberos.map(barbero => (
                  <div key={barbero.id} onClick={() => setBarberoId(barbero.id)} className={`cursor-pointer border rounded-xl p-4 flex items-center gap-3 transition-all ${ barberoId === barbero.id ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-500/10 shadow-[0_0_15px_rgba(16,185,129,0.15)]' : 'border-slate-200 dark:border-slate-700/50 bg-white dark:bg-[#03070e] hover:border-slate-300 dark:hover:border-slate-500' }`}>
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs transition-colors ${barberoId === barbero.id ? 'bg-emerald-500 text-white dark:text-[#03070e]' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'}`}>{barbero.name.substring(0,1).toUpperCase()}</div>
            ...
            <div>
              ...
              {!barberoId || !fecha ? (
                <div className="text-center p-5 border border-dashed border-slate-300 dark:border-slate-700/80 rounded-xl text-slate-500 text-sm font-medium">Selecciona especialista y fecha.</div>
              ) : cargandoHoras ? (
                <div className="text-center p-5 text-emerald-600 dark:text-emerald-400 text-sm font-bold animate-pulse">Verificando agenda...</div>
              ) : (
                <div className="grid grid-cols-3 gap-3 animate-fade-in">
                  {horariosBase.map(h => {
                    ...
                    const yaPaso = esHoy && (hHora < horaActual || (hHora === horaActual && minutosActuales >= 0));
                    ...
                    return (
                      ...
                      >
                        <span className={`font-medium ${bloqueado ? 'line-through decoration-rose-400/50' : ''}`}>{h}</span>
                        {bloqueado && (
                          <span className="text-[9px] font-bold uppercase tracking-wider text-rose-500 dark:text-rose-400/80 mt-0.5">
                            {estaOcupadoEnDB ? 'Reservado' : 'No disp.'}
                          </span>
                        )}
                      </div>
                    );
